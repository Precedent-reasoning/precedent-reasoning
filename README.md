...
**Key design decision:** Pre-indexed local corpus, no live scraping. A one-time (then periodic) ingestion job (`backend/ingest.py`) downloads the [Open Australian Legal Corpus](https://huggingface.co/datasets/isaacus/open-australian-legal-corpus) by Umar Butler (Isaacus) — licensed [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/) — filters it to NSW + Commonwealth decisions, and builds a local hybrid search index. The agent queries that local index on every request instead of scraping any court website live — faster, and immune to source-site rate limits or downtime. **If you self-host and deploy publicly, see [`DATA_LICENSE.md`](DATA_LICENSE.md)** — the CC BY 4.0 attribution obligation and each source court's own reproduction terms apply to your deployment too; this repo's Apache-2.0 `LICENSE` covers the code, not the data.
